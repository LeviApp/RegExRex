  ...
  lessons = [
    {
      ...
      "lesson_main": "Welcome to the RegEx Rex course. RegEx (Regular Expressions) is a way to search for patterns in text. You can search for letters, words, and digits. You can search for a certain length of words and a lot of other patterns. In standard searches, we use literal characters. Searching for 'code' tells the computer to look for the exact pattern of letters that spell out the word. In the first couple of lessons, we will be focusing on literal characters to explore different ways that RegEx can be used. While we are starting with simple concepts, we will jump into full-blown RegEx syntax shortly. The first pattern we will discuss is positioning, specifically, at the beginning. RegEx can be used to search for a pattern at the very start of a text block. Use the search bar above to solve the lesson's challenge. Using literal characters, search for the very first word in the following paragraph. (Ignore the second text box for now.)",
    ...
    },
    {
      ...
      "lesson_main": "The next pattern that deals with positioning is the end. RegEx can find a pattern in the last spot of a text string. It doesn't matter what is at the end of a text block. It could be letters, words, numbers, or other characters. Punctuation is always good to consider when searching strings. This is important for the beginning, middle, or end of a text block. Using a literal pattern, find the last word, including punctuation, in the following paragraph.",
      "lesson_challenge": "While all websites will ultimately be using HTML, CSS, and JavaScript, developers can still implement technologies, called frameworks, to help them build websites. Frameworks can be used to build the Front End. Every framework has foundational code it implements under the hood. The three most popular frameworks for the Front End are React, Angular, and Vue. All three of these frameworks are based in the JavaScript programming language!",
    ...
    },
    {
      ...
      "lesson_main": "RegEx allows you to use literal characters, but many times you will use special characters. However, there will be times you want to find these special characters literally. In order to do so, you must escape them. First, here is a list of special characters that RegEx recognizes.&nbsp;&nbsp;&nbsp;&nbsp;.&nbsp;&nbsp;&nbsp;&nbsp;*&nbsp;&nbsp;&nbsp;&nbsp;+&nbsp;&nbsp;&nbsp;&nbsp;?&nbsp;&nbsp;&nbsp;&nbsp;(&nbsp;&nbsp;&nbsp;&nbsp;)&nbsp;&nbsp;&nbsp;&nbsp;{&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;&nbsp;&nbsp;&nbsp;[&nbsp;&nbsp;&nbsp;&nbsp;]&nbsp;&nbsp;&nbsp;&nbsp;^&nbsp;&nbsp;&nbsp;&nbsp;\\&nbsp;&nbsp;&nbsp;&nbsp;$&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;What if you want to use a literal question mark or a literal dollar sign? You will have to escape these characters. In order to search for characters like this, you will almost always have to add a backslash (\\) before the symbol.&nbsp;&nbsp;&nbsp;&nbsp;\\[&nbsp;&nbsp;&nbsp;&nbsp; will search for a literal left square bracket.&nbsp;&nbsp;&nbsp;&nbsp;\\.&nbsp;&nbsp;&nbsp;&nbsp; will find a literal dot.&nbsp;&nbsp;&nbsp;&nbsp;\\$ will find a literal dollar sign. Your task will be to grab this RegEx expression using escaped characters.",
      "lesson_challenge": "[^$]",
      "lesson_solution": "/\\[\\^\\$\\]/"
    },
    {
    ...
    },
    {
      ...
      "lesson_main": "Flags are characters you add at the end of a RegEx expression that provide extra functionality. There are other flags in RegEx. For the sake of simplicity though, we will only be using the global and case-insensitive flags. You can combine flags in a RegEx expression to allow for more functionality. Example 1: /SCIENCE/. This will only find the first item that matches the exact word, 'SCIENCE'. Example 2: /SCIENCE/gi. This example will find all items matching the pattern 'SCIENCE', regardless of the case of the letters. You are editing a Computer Science article and realize the original writer wrote the word 'JavaScript' in many different ways. In order to standardize the form of the word, find them all in the following paragraph.",
    ...
    },
    {
      ...
      "lesson_main": "We have been using literal characters up to this point. Now, we will start to introduce RegEx symbols that match certain text qualities. The first symbol we will discuss is the star (*). The * value will match zero or more of the previous character or group. Example 1: /1234/. This will match the first instance of the number sequence, '1234'. Example 2: /12*3*4/. This RegEx expression will grab the first instance of a string that has a 1 followed by zero (0) or more 2's followed by zero (0) or more 3's followed by a 4. Here are some items that would match this pattern: 1234, 12234, 122234, 124, 134, 1222334, 122222224, 14 (because it matches the requirement of zero 2's and zero 3's). Keep order in mind when creating a RegEx pattern. Also, remember the flags that were just learned. You are needing to fix some mistyped phrasing for a children's book about tennis balls. The letters o and i failed to work or were over-sensitive. Find all of the instances of the sound of balls bouncing.",
      "lesson_challenge": "This is the sound of one tennis ball bouncing. boooiiiiiing, boiiing, bong, biiiing. Two tennis balls sound like this. Boing, Boooooooing, Booiing, Booooooong, Booiiiiiiiing, Bing, Booooooooing, Boiiiing. WATCH OUT! There are a pack of tennis balls chasing after you. BOOING, BOOOOOING, BOING, BONG, BOOOOOIIIIIIIIING, BOIIIIIIIING, BOOOIIING, BOOING, BOOOING, BING, BOIIING, BOOOOOOING, BOOOOOOIIIIIIING, BOING, BNG, BOOIIIIIIIIING!",
      "lesson_solution": "/bo*i*ng/gi"
    },
    {
      "lesson_id": 9,
      "lesson_number": 9,
      "lesson_title": "Plus!",
      "lesson_main": "We have been using literal characters up to this point. Now, we will start to introduce RegEx symbols that match certain text qualities. The second symbol we will discuss is the plus symbol (+). The + value will match one or more of the previous character or group. Example 1: /XY/g will find every instance of 'X' immediately followed by 'Y'. This will match all values that match 'XY' in the string. Example 2: /X+Y+/g. This RegEx expression will grab all values that have one (1) or more X's followed by one (1) or more Y's. Here are some items that would match this pattern: XY, XXY, XYY, XXYY, XXXY, XYYYYYYYY. Keep order in mind when creating a RegEx pattern. The example RegEx pattern will not match YYXX. Also, remember the flags that were just learned. You need to remove all the minus signs in the following math paragraph because the formulas are incorrect. First you must find them.",
    ...
    },
    {
      ...
      "lesson_main": "We have discussed the * symbol (0 or more characters), + symbol (1 or more characters), and ? symbol (0 or 1 character). What if you want to find a certain number of characters? You can use curly braces { }. Example 1: /M/g. This RegEx expression will find all the single values of 'M' in the string. Example 2: /Mm{4}/g. This pattern will match all values that equal Mmmmm. M followed by 4 m's. You are sleeping on the job! Select all the separate Z sleep cycles using the concept of the count.",
    ...
    },
    {
      ...
      "lesson_main": "We have discussed the * symbol (0 or more characters), + symbol (1 or more characters), and ? symbol (0 or 1 character). What if you want to find a certain range of characters? Example 1: /M/g. This RegEx expression will find all the single values of 'M' in the string. Example 2: /Mm{3,7}/g. This will match 'Mmmm', 'Mmmmm', 'Mmmmmm', 'Mmmmmmm', and 'Mmmmmmmm'. The regex will match all strings that start with a capital M, followed by a total of 3 - 7 m's. You wrote a list of all batteries you counted in your store. Unfortunately, you did not write down the amount you had of each type of battery. Find all the double A, triple A, and quadruple A batteries using the new RegEx concept. Remember not to put a space in the curly braces.",
    ...
    },
    {
      ...
      "lesson_main": "There is one more way to determine how many characters you want. If you want to grab items of a certain length or more, it is quite similar to the range discussed in the previous lesson. /Bo{5,}!/g will grab all shouts of disapproval that have at least 5 o's following the initial B, including the exclamation point at the end. You simply type in the lower boundary number and put a comma (,) after it. You are tallying up all the points for the ping-pong playoffs. You need to count up points from all the teams that qualify for the finals. The teams need to have made at least ten points.",
    ...
    },
    {
      ...
      "lesson_main": "RegEx was designed to match patterns. We have discussed patterns that match amounts of letters, like + and *. Now, we will discuss another type of pattern you can match letters with. These are considered general patterns called meta-characters. The first one I will mention is the space. Certainly you can type in a literal space and match spaces, but you can also grab all spaces using a general pattern. In most cases, a meta-character will be preceded by a backslash (\\). To find a space, a RegEx meta-character would look like this. /\\s/. This will grab the first single space. Like any literal character, you can use multiple general characters. /\\s\\s/ will match the first double space. Any other special pattern, like the * and +, discussed already will also work for meta-characters. You are editing a paragraph, and realize that the writer added two or more consecutive spaces within the paragraph. Your job is to find these places using the space meta-character.",
    ...
    },
    {
      ...
      "lesson_main": "The next important idea parentheses are used for is groupings. What if you want to apply RegEx to a group? This is where you would use parentheses. /([a-z]+\\s){10}/i will grab the first 10 words of a text block, along with their trailing spaces. RegEx first grabs the expression in the parentheses making it a group, in this case, it is grabbing a word and a space after. Then, RegEx will find that pattern repeated 10 times. Using groups, your challenge will be to grab sequences of three or four numbers that are four digits in length or less. Make sure your group includes the trailing space after the number.",
    ...
    },
    {
      ...
      "lesson_main": "Parentheses used as groups give you an important ability when searching. What happens if you want to search for a pattern, but only include part of it? This is where the concept of look behind and look ahead is important. First, we will discuss positive look behind. /(\\w+[.-]?\\w+@)(\\w+\\.\\w+)/g will grab an entire email address. It would grab the email curious-elephant@denverzoo.org. The RegEx also includes two captured groups. The first group will capture everything up to and including the @ symbol. It would grab curious-elephant@ from the example. The second captured group gets the domain name. In this case, denverzoo.org. This is wonderful if you want the entire address. However, what if you just want the domain name? The only extra RegEx you add is, on the group you do not want to be captured, add ?<= to the beginning, inside the parentheses. /(?<=\\w+[.-]?\\w+@)(\\w+\\.\\w+)/g This is called positive look behind. RegEx looks for a pattern that matches an email, and treats the first group as a condition rather than including it in the result. Keep in mind that this syntax only applies to groups preceding the pattern you want to match. You are putting a list together and want to group everyone based on their titles. You need to find all the names of the doctors, excluding their title.",
    ...
    },
    {
      ...
      "lesson_main": "Parentheses used as groups give you an important ability when searching. What happens if you want to search for a pattern, but only include part of it? This is where the concept of look behind and look ahead is important. We will discuss negative look behind. Instead of checking if a pattern exists, it ensures a specific pattern does not exist before your match. It treats the first group as a 'forbidden' condition. /(\\b)ship(\\b)/g will grab every instance of the word ship. This may be what you want to do. However, what if you want to see all the times it is being used as a suffix? Add a ?&#60;! to the group you want to exclude from the results. /(?&#60;!\\b)ship(\\b)/g will grab all of the values where ship is being used as a suffix. RegEx will look inside the captured pattern. In this case, it contains a word boundary \\b. Then, it will make sure all found values of ship are not preceded by that word boundary. The search results exclude every whole word ship, and finds every other time it is used as a suffix. You are creating a dummy design for a book and need to grab all numbers except for Volume numbers.",
    ...
    },
    {
      ...
      "lesson_main": "Parentheses used as groups give you an important ability when searching. What happens if you want to search for a pattern, but only include part of it? This is where the concept of look behind and look ahead is important. We will discuss positive look ahead. What if you want to grab the first part of a pattern and not the second part? This is what look ahead does.  /\\bextra(\\w{4,})/g will grab every word that has extra as its prefix and at least 4 letters after the prefix. To grab only the prefix 'extra', you can use positive look ahead. Use ?= for the captured group you want to exclude, which is located after another group you want to select. /\\bextra(?=\\w{4,})/g will grab every prefix 'extra' only if it is part of a longer word. You are given a list of files and their extensions. Using positive look ahead, grab every name of the file excluding the extension.",
    ...
    },
    {
      ...
      "lesson_main": "Parentheses used as groups give you an important ability when searching. What happens if you want to search for a pattern, but only include part of it? This is where the concept of look behind and look ahead is important. We will discuss negative look ahead. What if you want to grab a pattern only if a certain second part is missing? This is what negative look ahead does.  /\\d+(?!\\sbillion)\\b/gi will find all strings of numbers that are not followed by a billion. This may be useful if you want to isolate in a provided list the income of everybody except billionaires. Your task is to find all the team names that are disqualified from the RegEx competition. To be safe from disqualification, a team must make at least ten points. Hint: You need to match the whole word.",
    ...
    },
    {
      ...
      "lesson_main": "Parentheses used as groups give you an important ability when searching. What happens if you want to search for a pattern, but only include part of it? This is where the concept of look behind and look ahead is important. You can combine both look behind and look ahead in complex RegEx expressions. /(con)\\w+(ous)/g will grab the entire words that have the prefix con- and the suffix -ous. /(?<=con)\\w+(?=ous)/g will only grab the letters in the middle of the prefix con- and the suffix -ous. The RegEx would ignore words if they are missing either the prefix or the suffix. Words like 'conflict', 'condition', 'nervous', and 'fabulous' would be examples. In contrast, words like 'contagious', 'contentious', 'contiguous', and 'continuous' would be considered. RegEx will find the look ahead and look behind patterns and only grab the letters in between the prefix and suffix. 'tagi', 'tenti', 'tigu', and 'tinu' respectively. You are teaching a math lesson about Order of Operations. Using a combination of look behind and look ahead, you need to grab all of the innermost math expressions within the parentheses.",
