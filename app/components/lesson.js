...
export default class LessonComponent extends Component {
    ...
    lesson = {
        ...
        4: ["Lesson 4: Flags -- Flags are characters you add at the end of a RegEx expression that provide extra functionality. The first flag we will discuss is the global ('g') flag. A proper RegEx expression looks like this. Example 1: /website/. This example will only find the first item that matches the pattern. In this case, the first time the word 'website' is found in the text block. To find all items that match a given RegEx pattern, you must add the global flag at the end. Example 2: /website/g. This example will find all the items that match 'website' in the text block. In the following paragraph", "Frameworks can also be used for the Back End. A popular Back End framework is Node.js. It implements JavaScript behind the scenes. A framework that uses Python as its base computer language is Django. This framework includes a lot of out-of-th-box functionality. It even has a user-interface for admins dealing with server data. Other frameworks developers use include Laravel (PHP framework), Spring Boot (Java framework), and .NET (C# framework).", "framework/g"]
    }
